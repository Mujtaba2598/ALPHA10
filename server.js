...
app.post('/api/admin/approve-user', authenticate, (req, res) => {
    ...
    const pending = readPending();
    if (!pending[email]) return res.status(404).json({ success: false, message: 'User not found in pending' });
...
app.get('/api/admin/user-balances', authenticate, async (req, res) => {
    ...
    for (const [email, u] of Object.entries(users)) {
        ...
        try {
            ...
            balances[email] = { spot, funding, total: spot + funding, hasKeys: true, lastUpdated: new Date().toISOString() };
        } catch {
